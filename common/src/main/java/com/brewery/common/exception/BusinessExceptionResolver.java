	...
	public Mono<List<GraphQLError>> resolveException(Throwable exception, DataFetchingEnvironment environment) {
		...
		if (exception instanceof BusinessException)
			error = GraphqlErrorBuilder.newError()
				.errorType(((BusinessException) exception).getErrorType())
				.extensions(((BusinessException) exception).getExtensions())
				.message(exception.getMessage())
				.build();
		else {
			var internalServerError = new BusinessException(ExceptionReason.INTERNAL_SERVER_ERROR);
			error = GraphqlErrorBuilder.newError()
				.errorType(internalServerError.getErrorType())
				.extensions(internalServerError.getExtensions())
				.message(exception.getMessage())
				.build();
		}
